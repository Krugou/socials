...
const createParticleStore = () => {
  ...
  const colors = ['rgba(139, 92, 246, 0.5)', 'rgba(59, 130, 246, 0.5)', 'rgba(255, 130, 246, 0.5)'];
      ...
      try {
        ...
        return particles.map((p): Particle => {
          ...
          if (!isValidParticle(updatedParticle)) {
            throw new ParticleError(`Invalid particle update: ${JSON.stringify(updatedParticle)}`);
          }
