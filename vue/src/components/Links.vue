<template>
  <div
    :class="[
      'perspective relative space-y-4 p-4 rounded-2xl shadow-xl',
  ...
  >
    <template v-if="visible">
      <a
        ...
        :class="[
          'group relative block overflow-hidden rounded-lg p-2 text-center backdrop-blur-sm transition-all duration-300 md:p-4 border',
          isDarkMode
      ...
      >
        <div class="relative z-10 flex items-center justify-center space-x-2 transition-transform duration-300 group-hover:scale-105">
          <img
            :src="link.icon"
            :alt="link.title"
        ...
        <div
          :class="[
            'absolute inset-0 -z-10 transition-opacity duration-300 opacity-0 group-hover:opacity-100',
            isDarkMode
    ...
    <template v-else>
      <div
        :class="['h-[68px] animate-pulse rounded-lg', isDarkMode ? 'bg-card/40' : 'bg-white/5']"
      ></div>
      <div
        :class="['h-[68px] animate-pulse rounded-lg', isDarkMode ? 'bg-card/40' : 'bg-white/5']"
      ></div>
