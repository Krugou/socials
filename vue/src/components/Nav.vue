    ...
    <div class="container mx-auto max-w-7xl">
      <ul class="flex items-center justify-between">
        ...
        <div class="flex items-center justify-center space-x-2">
          ...
          <li>
            <span v-if="weatherLoading" class="ml-2 text-xs animate-pulse">
              Loading weather...
            </span>
            <span v-else-if="weatherError" class="ml-2 text-xs text-red-300" :title="weatherError">
              🌧️
            </span>
            <span v-else-if="weather" class="ml-2 flex items-center text-xs" :title="weather.description">
...
const handleNavClick = async (link: NavLink) => {
  try {
    await logNavigationEvent({
      navHref: link.href,
      navText: getTranslation(link, props.language).text,
...
onMounted(async () => {
  ...
  try {
    ...
    weather.value = await fetchNorwayWeather(lat, lon);
  } catch (error) {
    weatherError.value = error instanceof WeatherError ? error.message : 'Weather unavailable';
  } finally {
